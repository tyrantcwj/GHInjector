...
bool update_injector(const std::wstring & newest_version, bool & ignore, InjectionLib * Lib)
{
	...
	if (cmp > 0)
	{
		update_msg = L"当前 GH Injector 版本已过期。\n最新版本是 V";
		update_msg += newest_version;
		update_msg += L".\n";
		update_msg += L"是否更新？";

		parent.setWindowTitle("有新版本可用");
		box->addButton(QMessageBox::Ignore);
	}
	else if (cmp == 0)
	{
		update_msg = L"你已经在使用最新版本的 GH Injector。\n是否重新下载 V";
		update_msg += GH_INJ_GUI_VERSIONW;
		update_msg += L"?";

		parent.setWindowTitle("重新下载");
	}
	else
	{
		StatusBox(false, "版本检查出现问题。\n你的版本号看起来来自未来。");
		parent.close();
		delete box;
	...
	DownloadProgressWindow * UpdateWnd = new(std::nothrow) DownloadProgressWindow(QString("正在下载 V") + QString::fromStdWString(newest_version), labels, "正在初始化...", 250, Q_NULLPTR);
	...
	if (ret > 0)
	{
		error_msg = "发生未知错误。请检查新版本是否已成功下载。\n错误代码 = ";
	...
	else if (ret < 0)
	{
		switch (ret)
		{
			case UPDATE_ERR_DOWNLOAD_FAIL:
			{
				...
				error_msg = "URLDownloadToFileW 失败，错误码 0x";
				error_msg += QString::fromStdString(s_hr);
			}
			break;

			case UPDATE_ERR_DELETE_FAIL:
				error_msg = "删除旧文件失败。请关闭注入器并手动解压新版本。";
				break;

			case UPDATE_ERR_RENAME_FAIL:
				error_msg = "重命名旧文件失败。请关闭注入器并手动解压新版本。";
				break;

			case UPDATE_ERR_UNZIP_FAIL:
				error_msg = "解压新文件失败。请关闭注入器并手动解压新版本。";
				break;

			case UPDATE_ERR_INCOMPLETE:
				error_msg = "下载不完整或解压失败。请关闭注入器并手动解压新版本。";
				break;

			case UPDATE_ERR_LAUNCH_FAIL:
				error_msg = "启动新版本失败。请关闭注入器并手动启动新版本。";
				break;
		...
		if (ret == UPDATE_ERR_DELETE_FAIL || ret == UPDATE_ERR_RENAME_FAIL || ret == UPDATE_ERR_UNZIP_FAIL || ret == UPDATE_ERR_INCOMPLETE || ret == UPDATE_ERR_LAUNCH_FAIL)
		{
			...
			if (shell_ret <= 32)
			{
				...
				error_msg = "打开文件位置失败。请手动打开以下路径并启动/解压新版本:\n";
...
void update_update_callback(DownloadProgressWindow * hProgressWindow, void * pData)
{
	...
	else if (!data->download_finished)
	{
		...
		else
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				hProgressWindow->SetStatus("正在移除旧文件...");

				data->download_finished = true;
			}
			else
			{
				std::string new_status = "URLDownloadToFileW 失败，错误码 0x";
	...
	else if (!data->cleanup_finished)
	{
		...
		{
			hProgressWindow->SetStatus("正在解压新文件...");
	...
	else if (!data->unzip_started)
	{
		...
		{
			hProgressWindow->SetStatus("解压文件失败...");
	...
	else if (!data->unzip_finished)
	{
		...
		else
		{
			...
			if (all_files_unzipped)
			{
				...
				hProgressWindow->SetStatus("正在启动更新后的版本...");
			}
		}
	}
	else if (!data->launch_finished)
	{
		...
		{
			hProgressWindow->SetStatus("启动更新后的版本失败...");
